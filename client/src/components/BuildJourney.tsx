...
import { Check, ChevronLeft, ChevronRight, Compass, House, ImagePlus, LocateFixed, MapPin, Send, WalletCards, X } from "lucide-react";
import "./BuildJourney.css";

type Coordinates = { lat: number; lng: number } | null;
type ProjectForm = { name: string; email: string; phone: string; location: string; province: string; budget: string; buildType: string; style: string; message: string; referenceImages: string[] };

const initialForm: ProjectForm = { name: "", email: "", phone: "", location: "", province: "", budget: "", buildType: "Vivienda familiar", style: "Contemporánea", message: "", referenceImages: [] };
...
export default function BuildJourney() {
  ...
  const uploadReferences = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.some((file) => !["image/jpeg", "image/png", "image/webp"].includes(file.type) || file.size > 3_000_000)) { setError("Usa imágenes JPG, PNG o WebP de hasta 3 MB."); return; }
    try {
      const uploads = await Promise.all(files.map(async (file) => {
        ...
        return uploadMutation.mutateAsync({ filename: file.name, mimeType: file.type as "image/jpeg" | "image/png" | "image/webp", base64 });
      }));
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      await leadMutation.mutateAsync({ leadType: "construction", propertyId: null, name: form.name, email: form.email, phone: form.phone || null, preferredLocation: form.location, preferredProvince: form.province || null, latitude: coordinates ? String(coordinates.lat) : null, longitude: coordinates ? String(coordinates.lng) : null, budget: form.budget || null, referenceImages: form.referenceImages, attributionCode: "MARTINEZ", message: `Proyecto: ${form.buildType}. Estilo: ${form.style}. ${form.message}` });
